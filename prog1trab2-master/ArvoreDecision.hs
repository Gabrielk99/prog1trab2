module ArvoreDecision where
    ...
    iGR base tamBase (Numerico (nome,atributo):caract) classes classe |iv/=(trace ("iv = "++(show iv)++"\n"++"ig="++(show ig)) 0) =  (ig/iv,(Numerico (nome,atributo))):iGR base tamBase caract classes classe
    ...
    valoresIGnumerico nome (atb:atributos) base tamBase classes classe |tamConjunto /=0 && tamBase /=0 = ((tamConjunto/(fromIntegral tamBase))*(trace("entropia = "++(show entropiaX))entropiaX))+(valoresIGnumerico nome atributos base tamBase  classes classe) 
    ...
    entropia base tamBase (ci:classes) classe |percentagem /=0 = -(percentagem)*(logBase(2)(percentagem)) + entropia base tamBase classes classe
                            ...
                            where 
                            percentagem = (fromIntegral(filtraClasse ci base classe))/(tamBase) 
    ----------------------------------------------------------------------------------------------------------------
