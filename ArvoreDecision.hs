module ArvoreDecision where
    ...
    avaliaValores (Numerico (nome,atributo)) base caracteristicas classes maisComum = [Valor (convertToDecisaoNum (a,b)) (arvoreDecisao (filtraAtributoNum nome (a,b) base) caracteristicas maisComum classes) | (a,b)<-atributo]                                       
    ------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    arvoreDecisao base caracteristicas maisComum classes |null base = Resultado maisComum
                                                ...
                                                where 
                                                 tamBase = length base
                                                 discretizado = discretiza caracteristicas base classes
    ...
    iGR base tamBase (Numerico (nome,atributo):caract) classes |ig/=0 && iv/= 0 =  (ig/iv,(Numerico (nome,atributo))):iGR base tamBase caract classes
                                                               |otherwise = (0,(Numerico (nome,atributo))):iGR base tamBase caract classes
                                 where 
                                ig = iGnumerico base tamBase nome atributo (tail classes) 
